...
<li><a href="{if $page->cf_urlsm_string eq NULL}{$HOME}{$page->urlKey}{else}{$HOME}{$page->cf_urlsm_string}{/if}">{$page->name|strip_tags|truncate:$params.length}</a></li>
    ...
    {menuNormalTree pages=$data.pagesDB->getPagesByParent($page->id,$params.where,$params.sort,'*',$params.limit,null,true)}        
                    {/if}{else}
   <i class="fa wm-color fa-check"></i><li><a href="{if $page->cf_urlsm_string eq NULL}{$HOME}{$page->urlKey}{else}{$HOME}{$page->cf_urlsm_string}{/if}">{$page->name|strip_tags|truncate:$params.length}</a> </li>
          {/if}
        {else}
<i class="fa wm-color fa-check"></i><li><a href="{if $page->cf_urlsm_string eq NULL}{$HOME}{$page->urlKey}{else}{$HOME}{$page->cf_urlsm_string}{/if}">{$page->name|strip_tags|truncate:$params.length}</a> </li>
        {/if} {/foreach}
